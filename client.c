...
int main (int argc, char **argv) {
  ...
  ///  TODO: preparing sockaddr_in
  
  bzero(&svr_addr, sizeof(svr_addr));
  svr_addr.sin_family = AF_INET;// protocol 
...
void connection_handler (int sockfd) {
    ...
    ///  TODO: read file list from server
    
	int read_bytes;
	read_bytes= read(sockfd, buf, strlen(buf));
	buf[read_bytes] = '\0';
	printf("%s\n",buf);
    ...
    while (scanf(" %s", filename) > 0) {
        ...
        /// TODO: send requested file name to server
        
	int write_bytes;
	write_bytes=write(sockfd, filename, strlen(filename));
  ...
  if (fp) {
      while (read_sum < file_size) {
        memset(buf, '\0', MAX_SIZE);
          ...
          ///TODO: receive file data from server
